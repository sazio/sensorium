...
class VOneBlock(nn.Module):
    def __init__(self, sf, theta, sigx, sigy, phase,
                 ...
                 simple_channels=128, complex_channels=128, ksize=25, stride=4, input_size=(144, 256)): 
                 #  input_size=224):
        super().__init__() #

        self.in_channels = 1 #for sensorium --> 4 for sensorium +
        ...
        self.input_x_size = input_size[0]
        self.input_y_size = input_size[1]
    ...
    def fix_noise(self, batch_size=256, seed=None):
        ...
        noise_mean = torch.zeros(batch_size, self.out_channels, int(self.input_y_size/self.stride),
                                 int(self.input_x_size/self.stride))
        if seed:
